...
MotorProtocol::MotorProtocol(char _motorId, QObject *parent)
    : QObject(parent), motorId(_motorId) {}
...
char MotorProtocol::hexSum(const QByteArray &byteArray, const int st,
                           const int ed) {
  char tem = 0x00;
  for (int i = st; i <= ed; i++) {
    tem += byteArray[i];
